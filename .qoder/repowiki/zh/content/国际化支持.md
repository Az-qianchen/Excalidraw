...
- [packages/excalidraw/actions/actionMagicWand.tsx](file://packages/excalidraw/actions/actionMagicWand.tsx)
...
**所做更改**
- 添加了魔法抠图工具的国际化支持说明
- 新增了阈值设置、羽化半径、反转/保持选择模式等新功能的本地化支持
- 更新了翻译键值管理和本地化流程的相关内容
- 完善了魔法抠图工具相关的技术细节和最佳实践
...
**新增** 本版本特别增加了对魔法抠图工具的国际化支持说明，包括阈值设置、羽化半径、反转/保持选择模式等新功能的本地化实现。
...
- 翻译平台配置：crowdin.yml
- 魔法抠图工具功能：packages/excalidraw/actions/actionMagicWand.tsx
...
ACTION["actionMagicWand.tsx<br/>魔法抠图工具"]
end
subgraph "资源与工具"
LOCALES["locales/*.json<br/>翻译文件"]
COV["build-locales-coverage.js<br/>覆盖率统计"]
CROWDIN["crowdin.yml<br/>Crowdin 配置"]
TYPES["types.ts<br/>魔法抠图状态类型"]
MWTOOL["magic-wand-tool.d.ts<br/>工具类型定义"]
end
LL --> LS
LS --> LD
LS --> I18N
...
- [packages/excalidraw/actions/actionMagicWand.tsx:1-61](file://packages/excalidraw/actions/actionMagicWand.tsx#L1-L61)
- [packages/excalidraw/types.ts:244-286](file://packages/excalidraw/types.ts#L244-L286)
- [packages/excalidraw/mask-editor/magic-wand-tool.d.ts:1-42](file://packages/excalidraw/mask-editor/magic-wand-tool.d.ts#L1-L42)
...
**更新** 魔法抠图工具通过 actionMagicWand.tsx 集成国际化，使用 helpDialog.magicWandStart 作为翻译键值。
...
- [packages/excalidraw/actions/actionMagicWand.tsx:13-61](file://packages/excalidraw/actions/actionMagicWand.tsx#L13-L61)
...
### 功能概述
魔法抠图工具是 Excalidraw 中的一个高级图像处理功能，允许用户通过颜色阈值、羽化效果和选择模式来智能地选择和抠取图像区域。该工具完全支持国际化，所有用户界面元素都通过翻译键值进行本地化。

### 翻译键值管理
- **基础功能翻译**
  - `helpDialog.magicWandStart` - 魔法抠图工具的启动按钮标签
- **参数设置翻译**
  - 阈值设置相关的翻译键值（如 `magicWand.threshold`）
  - 羽化半径相关的翻译键值（如 `magicWand.featherRadius`）
  - 选择模式相关的翻译键值（如 `magicWand.inverted`、`magicWand.contiguous`）

### 状态管理与本地化集成
魔法抠图工具的状态通过 AppState 管理，包括：
- `magicWandElementId` - 当前激活的魔法抠图元素 ID
- `magicWandThreshold` - 颜色阈值设置
- `magicWandFeatherRadius` - 羽化半径设置
- `magicWandInverted` - 反转选择模式
- `magicWandContiguous` - 连续选择模式

这些状态值在不同语言环境下保持一致的数据结构，但用户界面文本通过翻译系统进行本地化。

### 类型定义与国际化
魔法抠图工具的类型定义位于 `magic-wand-tool.d.ts`，包括：
- `ImageData` 接口 - 图像数据结构
- `Mask` 接口 - 掩码数据结构
- `Contour` 接口 - 轮廓数据结构
- `MagicWand` 模块 - 核心算法接口

这些类型定义支持多语言环境下的数据处理和显示。

### 集成实现
魔法抠图工具通过 `actionMagicWand.tsx` 实现与国际化系统的集成：
- 使用 `t()` 函数获取本地化文本
- 通过 `PanelComponent` 提供本地化的工具面板
- 支持动态语言切换时的文本更新

```mermaid
flowchart TD
Start(["魔法抠图工具初始化"]) --> CheckLang["检查当前语言"]
CheckLang --> GetText["使用 t() 获取本地化文本"]
GetText --> SetButton["设置按钮标签"]
SetButton --> ShowPanel["显示工具面板"]
ShowPanel --> UserInteraction["用户交互"]
UserInteraction --> UpdateState["更新魔法抠图状态"]
UpdateState --> ReRender["重新渲染界面"]
ReRender --> CheckLang
```
...
- 组件耦合
  ...
  - 魔法抠图工具通过 actionMagicWand.tsx 与国际化系统集成。
- 外部依赖
...
actionMagicWand["actionMagicWand.tsx"] --> I18N
actionMagicWand --> types["types.ts"]
types --> magicWandTool["magic-wand-tool.d.ts"]
```
...
- 魔法抠图工具性能
  - 阈值计算和羽化处理可能涉及大量像素操作，建议在大图像上使用适当的阈值设置以平衡质量和性能。
...
- 魔法抠图工具问题
  - 检查阈值设置是否合理、羽化半径是否过大、选择模式是否符合预期。
  - 确认图像元素是否正确选中且支持魔法抠图操作。
...
- [packages/excalidraw/actions/actionMagicWand.tsx:20-46](file://packages/excalidraw/actions/actionMagicWand.tsx#L20-L46)

## 结论
Excalidraw 的国际化体系以清晰的职责分离为基础：i18n.ts 负责语言注册、切换与回退，Trans 提供 JSX 级别的翻译能力，应用层通过 LanguageList 与状态管理实现用户可控的语言切换。配合 Crowdin 与覆盖率统计，项目实现了从源文件到云端翻译再到本地加载的完整闭环。

**新增** 魔法抠图工具作为高级功能，通过完善的国际化支持为用户提供多语言环境下的专业图像处理体验。工具的状态管理、类型定义和界面本地化都经过精心设计，确保在不同语言环境下的一致性和易用性。

对于更复杂的本地化需求（如数字/日期格式化），建议在业务层扩展使用 Intl API，并在主题层完善字体与布局适配。
...
### 魔法抠图工具翻译键值规范
- 基础键值前缀：`magicWand.`
- 参数设置键值：`magicWand.threshold`、`magicWand.featherRadius`
- 选择模式键值：`magicWand.inverted`、`magicWand.contiguous`
- 工具面板键值：`helpDialog.magicWandStart`

**章节来源**
- [crowdin.yml:1-4](file://crowdin.yml#L1-L4)
- [scripts/build-locales-coverage.js:1-38](file://scripts/build-locales-coverage.js#L1-L38)
- [packages/excalidraw/actions/actionMagicWand.tsx:15](file://packages/excalidraw/actions/actionMagicWand.tsx#L15)