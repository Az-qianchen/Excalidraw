...
B["binding.ts<br/>绑定/连接逻辑"]
BD["bounds.ts<br/>边界计算"]
C["collision.ts<br/>碰撞检测"]
end
subgraph "dev-docs/docs/@excalidraw/excalidraw/api"
S["excalidraw-element-skeleton.mdx<br/>元素骨架与转换"]
U["utils/utils-intro.md<br/>类型检查工具函数"]
end
...
## 架构总览
下图展示 ExcalidrawElement 的继承与组合关系，以及关键工具类型的作用范围。
...
## 结论
Excalidraw 的元素类型体系以 _ExcalidrawElementBase 为核心，通过联合类型与工具类型实现强类型约束与灵活扩展。版本控制、分数索引与状态字段共同保障协作与渲染的一致性。借助工具函数与骨架转换，开发者可以安全地扩展自定义元素类型，并将其无缝集成到现有工作流中。
